...
#define PCIE_REG_BASE	(PCIE_EXP_BASE)

#define PCIE_MISC_PCIE_STATUS	0x30
...
static uint64_t pci_segment_lib_get_base(uint32_t address, uint32_t offset)
{
	...
	if (address != 0U) {
		...
		VERBOSE("Address : bus %d dev %d fun %d \n", bus, dev, fun);
		...
		for(int i = 0; i < PCIE_CONFIG_SIZE; i+=0x10){
			// mmio_write_16(PCIE_REG_BASE + i, 0x13b5);
			status = mmio_read_16(PCIE_REG_BASE + i);
			if (status == 0x13b5){
				ERROR("Dev vendor id found %x \n\n\n\n", status);
				break;
			}
		}
		VERBOSE("PRINT STATUS : %x \n", status);
...
{	ERROR("entering pci_read_config");
	...
	if (base == INVALID_PCI_ADDR) {
		ERROR("INVALID_PCI_ADDR \n");
		*val = base;
	} else {
		switch (sz) {
		case SMC_PCI_SZ_8BIT:
			VERBOSE("In SMC_PCI_SZ_8BIT");
